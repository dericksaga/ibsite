...
$sql = mysql_query("SELECT * FROM galeria WHERE id_franquia='1' order by data desc");
}
...
<option value="?pg=galeria/admin/listar_fotos.php&caminho=images/eventos/<? echo $dados[id_franquia]?>/<? echo $dados[pasta]?>"><? $data=explode("-",$dados[data]); echo "$data[2]/$data[1]/$data[0]";?> - <? echo $dados[nome]?></option>
<? }?>
</select></form>
...
if($file != '.' && $file !='.' && $file !=''){ 
 ...
 ?>
  ...
  <td><? echo "<a href='$caminho/$file' target='_blank'><img style='border:1px solid #cccccc;' src='thumbs.php?w=70&h=52&imagem=$caminho/$file' border='0'></a>";?></td>
    ...
    <td align="center" valign="middle"><a href="<? if($tipo != "dir"){ echo "javascript:confirmaExclusao('?pg=galeria/admin/excluir_fotos.php&caminho=$caminho&nomedoarquivo=$file')";
 } else { echo "javascript:confirmaExclusao('?pg=galeria/admin/excluir_fotos.php&caminho=$caminho&nomedapasta=$file')";}?>"><img src="images/admin/botao_drop.png" alt="Excluir" border="0"></a></td>
