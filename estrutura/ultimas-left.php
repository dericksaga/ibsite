...
<? $query = mysql_query("SELECT * FROM noticias_dados where idcat = $cat1 order by id desc limit 1");
$dados = mysql_fetch_array($query); ?>
<div style="padding:0px; margin:0px; padding-top:0px; border-top:1px solid <?=$cor1?>; min-height:60px;" class="textinho">
<a href="?pg=noticia&id=<?=$dados[id]?>" style="display: block;"> <? 
if (is_file("images/noticias/$dados[id]/$dados[foto01]")){
	echo "<img src='thumbs.php?w=56&h=56&imagem=images/noticias/$dados[id]/$dados[foto01]' border='$dados[borda]' align='left' style='border:solid 1px gray; margin-right:5px;'>" ;
} else {
	echo "<img src='thumbs.php?w=56&h=56&imagem=images/logo70.jpg' align='left' style='margin-right:5px; ' border='0'>" ;
}
...
<div style="padding:0px; margin:0px; padding-top:0px; border-top:1px solid <?=$cor2?>; min-height:60px;" class="textinho">
<a href="?pg=noticia&id=<?=$dados[id]?>" style="display: block;"> <? 
if (is_file("images/noticias/$dados[id]/$dados[foto01]")){
	echo "<img src='thumbs.php?w=56&h=56&imagem=images/noticias/$dados[id]/$dados[foto01]' border='$dados[borda]' align='left' style='border:solid 1px gray; margin-right:5px;'>" ;
} else {
...
<div style="padding:0px; margin:0px; padding-top:0px; border-top:1px solid <?=$cor3?>; min-height:60px;" class="textinho">
<a href="?pg=noticia&id=<?=$dados[id]?>" style="display: block;"> <? 
if (is_file("images/noticias/$dados[id]/$dados[foto01]")){
	echo "<img src='thumbs.php?w=56&h=56&imagem=images/noticias/$dados[id]/$dados[foto01]' border='$dados[borda]' align='left' style='border:solid 1px gray; margin-right:5px;'>" ;
} else {
