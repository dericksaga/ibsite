...
<?
	$sql= mysql_query("SELECT * FROM fotos where id = $_GET[id] order by id desc limit 1");
	while ($dados=mysql_fetch_array($sql)){
        $sql2= mysql_query("SELECT * FROM fotos_galerias where id = $dados[codigo]");
        while ($dados2=mysql_fetch_array($sql2)){
    		echo "<div class='titulozinhos' style='padding-top:15px; margin-bottom:15px; width:200PX'><a href='?pg=fotos&cat=$dados2[id]' style='color:#a70d6f'>$dados2[nome]</a></div>";
        }
	...
	<?
        ...
        while ($dados2=mysql_fetch_array($sql2)){
    ?>
    	<div class="titulozinhos" style="padding-top:15px; border-bottom:1PX SOLID #a70d6f; width:480PX"><a style="color:#a70d6f">MAIS FOTOS DESTA GALERIA</a></div>
        <div style="padding-top:15px; width:480PX; text-align:center">
			<?
                $sql3= mysql_query("SELECT * FROM fotos where codigo = $dados2[id] order by id desc");
            ...
            ?>
                <a href="?pg=fotos2&id=<?=$dados3[id]?>" style="padding-right:10px"><img alt="<?=$dados3[coment]?>" src="<?=$foto?>" border="0" style="margin-top:10px;<?=$borda;?>" align="absmiddle"></a>
            <? $borda = "";} ?>
		</div>
	<? }} ?>
</div>
