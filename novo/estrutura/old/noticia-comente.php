...
<? if ($_GET[acao] == "enviar"){
	if ($_GET[validacao] == "3"){ 
		...
		$m = "Foi comentado na noticia do site www.$site, na postagem de t&iacute;tulo: <br>";
		...
		$m .= "<a href='http://www.$site/index2.php?pg=libera&acao=vai&cod=".md5($_GET[email])."&iiiiid=$id_recuperado'>http://www.$site/index2.php?pg=libera&acao=vai&cod=".md5($_GET[email])."&iiiiid=$id_recuperado</a>";
		...
		$ass = "Comentario no site www.$site - ";
		$ass .= time();
...
$busca = "SELECT * FROM noticias_comentarios where id_noticia=$_GET[id] and autorizacao = 1 order by id desc";
...
<? if($tr == 1){ $palavra = $tr." Coment&aacute;rio";} else {$palavra = $tr." Coment&aacute;rios";} ?>
	<div class="citacao" style="border-bottom:3px double gray;"><b><?=$palavra?>!!!</b></div><br />
<? } ?>
